...
// new로 동적할당한 객체가 파생 클래스
// 이 포인터가 가리키는 객체가 삭제될 때는, 기본 클래스 포인터를 통해 삭제 된다.
// 기본 클래스내 소멸자가 비가상 소멸자 

// 즉, 기본 클래스 포인터를 통해서, 파생 클래스 객체가 소멸될 때
// 그 기본 클래스에 비가상 소멸자가 들어있음ㄴ
// 파생 클래스 부분의 소멸자가 호출되지 않는다.'