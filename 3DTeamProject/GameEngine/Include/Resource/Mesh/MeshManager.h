...
public:
	...
	bool CreateMesh(Mesh_Type Type, const std::string& Name, void* VtxData, int Size, int Count,
		...
		DXGI_FORMAT Fmt = DXGI_FORMAT_UNKNOWN, class CScene* Scene = nullptr);
...
private :
	bool CreateSphere(std::vector<Vertex3D>& vecVertex, std::vector<int>& vecIndex,
		float Radius, unsigned int SubDivision);
