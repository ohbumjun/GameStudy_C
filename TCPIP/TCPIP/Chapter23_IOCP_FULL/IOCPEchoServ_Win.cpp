...
int main(int argc, char* argv[])
{
	...
	while (1)
	{
		...
		// 클라이언트 연결 요청에 대한 새로운 소켓에 대한 정보는
		// GetQueuedCompletionStatus 함수를 호출하는 CP 오브젝트에 할당된
		// 쓰레드에서 반환받게 된다.
		...
		// IOCP는 기본적으로 입려의 완료와 출력의 완료를 구분지어주지 않는다
		// 입력이건 출력이건 완료되었따는 사실만 인식시켜준다. 따라서 입력을 진행한 것인지, 출력을 진행한 것인지에 대한
		// 정보를 별도로 기록해둬야 한다.
		ioInfo->rwMode = READ;

		// 비동기 데이터 수신 (마지막 인자 NULL 주목 !)
		// &(ioInfo->overlapped) 해당 값은 GetQueuedCompletionStatus 함수가 반환을 하면서 얻을 수 있다.

		// 그런데 구조체 변수의 주소값은, 첫번째 멤버의 주소값과 동일하므로
		// PER_IO_DATA 구조체 변수의 주소값을 전달한 것과 같다.
...
// 해당 함수를 main으로 호출하는 쓰레드를 "CP 오브젝트에 할당된 쓰레드"라고 한다.
unsigned __stdcall EchoThreadMain(LPVOID CompleitionPortIO)
{
	...
	while (1)
	{
		...
		즉, 클라이언트 연결 요청 소켓에 대한 정보 + overlapped 구조체에 대한 정ㅂ조
