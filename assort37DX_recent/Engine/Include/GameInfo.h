...
struct AnimationCBuffer {
	int BoneCount;
	int CurrentFrame; // 현재 Animation 상에서 현재 Frame
	int NextFrame;     // 현재 Animation 상에서 다음 Frame
	float Ratio;          // Current Frame 과 Next Frame 사이의 비율 --> Key Frame 사이 어디즈음에 있는지
	int FrameCount;  // 해당 Animation 의 Bone 들이 몇 Frame 짜리들인가
	int RowIndex;     // 각 객체마다 Bone 변환 행렬 정보들을 ? 행렬 형태로 정보를 넣을 것이다
							 // 각 Row가 하나의 Bone 변환 행렬 정보
							 // 그 중에서 몇번째 Bone 인지 
	int ChangeAnimation;		// 현재 Animation을 바꿔야 하는지
	float ChangeRatio;			// Change 과정 중에 흐른 시간의 비율 ex) 2.5초 동안 Animation Change 가 발생하는데, 그 중에서 흐른  시간 --> 0 에서 1 사이의 값으로 표현
	int ChangeFrameCount;  // 바뀔 그 다음 Animation 의 Frame 개수
	Vector3 Empty;
};
struct LightCBuffer
{
