...
* - name : 롱 옵션의 이름
* - has_arg : 
    no_argument (혹은 0)          : 파라미티 취하지 않음
    required_argument (혹은 1) : 반드시 파라미터 필요
    optional_argument (혹은 2) : 파라미티 취할 수 있음

* - flag : 옵션을 인식하면, 해당 변수에 값을 대입한다.
* - val : 옵션을 인식하면, flag 에 대입할 값
*/
...
{
    argc = 1;
    char *tempArgv[] = { argv[0], "--debug", "http.cpp"};
    ...
    /*
    * getopt_long : 명령어 라인 옵션을 해석하는 함수
    * - '--' 로 시작하는 옵션을 해석한다.
    */
