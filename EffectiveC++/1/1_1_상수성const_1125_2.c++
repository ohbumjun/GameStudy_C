...
/*
부가 설명 : 상수 멤버 함수가 비상수 멤버함수를 호출하게 하면 안되는 것일까 ?
ex) operator[] --> const operator[]

상수 멤버 함수는 , 해당 객체의 논리적인 상태를 바꾸지 않겠다고
컴파일러와 굳게 약속한 함수인 반면

비상수 멤버함수는 이런 약속 같은 것을 하지 않는다.
즉, 어쩌다가 상수 멤버에서 비상수 멤버를 호출하게 되면
수정하지 않겠다고 약속한 그 객체를 배신하는 셈이 되고
그 객체는 변경될 위험에 빠질 수 있다.

그러므로, 상수 멤버함수에서 비상수 멤버함수를 호출하면
틀리다는 이야기가 된다. 
